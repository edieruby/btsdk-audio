...
static void hfp_ag_rfcomm_acceptor_opened( hfp_ag_session_cb_t *p_scb );
...
{
    char                *p = ( char * )p_data;
    hfp_ag_session_cb_t *p_scb = hfp_ag_find_scb_by_rfc_handle( port_handle, HFP_FLAG_RFCOMM_DATA );
...
{
    uint8_t i;
    hfp_ag_session_cb_t *p_scb = hfp_ag_find_scb_by_rfc_handle( port_handle, HFP_FLAG_RFCOMM_CONTROL );
 ...
 */
void hfp_ag_rfcomm_start_server( hfp_ag_session_cb_t *p_scb )
{
 ...
 */
void hfp_ag_rfcomm_do_close( hfp_ag_session_cb_t *p_scb )
{
    wiced_bt_rfcomm_result_t rfcomm_result;
 ...
 */
void hfp_ag_rfcomm_closed( hfp_ag_session_cb_t *p_scb )
{
    wiced_bt_rfcomm_result_t rfcomm_result;

    /* call appropriate close cback */
    if ( p_scb->state == HFP_AG_STATE_OPENING )
        hfp_ag_process_open_callback( p_scb, HCI_CONTROL_HF_STATUS_FAIL_RFCOMM );
    else
        hfp_ag_hci_send_ag_event( HCI_CONTROL_AG_EVENT_CLOSE, p_scb->app_handle, NULL );
...
static void hfp_ag_rfcomm_opened ( hfp_ag_session_cb_t *p_scb )
{
    ...
    hfp_ag_process_open_callback ( p_scb, HCI_CONTROL_HF_STATUS_SUCCESS );
 ...
 */
void hfp_ag_rfcomm_acceptor_opened( hfp_ag_session_cb_t *p_scb )
{
    uint16_t lcid;
    int      i;
    int      status;
