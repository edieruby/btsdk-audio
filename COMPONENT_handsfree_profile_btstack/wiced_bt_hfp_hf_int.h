...
#if WICED_BT_HFP_HF_DEBUG == TRUE
#define WICED_BTHFP_TRACE   WICED_BT_TRACE
#define WICED_BTHFP_ERROR   WICED_BT_TRACE
#else
#define WICED_BTHFP_TRACE(...)
#define WICED_BTHFP_ERROR(...)
#endif
...
#ifndef WICED_BT_HFP_HF_MAX_NUM_PEER_IND
...
#define WICED_BT_HFP_HF_NO_EVT 0
...
/* Callback function executed when a command is parsed */
...
typedef struct
{
    wiced_bool_t                   is_init;                       /* Whether Handsfree is initialized or not */
    ...
    uint16_t                       ag_profile_uuid;               /* Used to store AG profile ID for Initiator Role */
...
typedef struct  {
    ...
    uint8_t     scn;
    uint16_t    uuid;
} wiced_bt_hfp_rfcomm_server_data_t;
...
extern wiced_bt_hfp_rfcomm_server_data_t wiced_rfcomm_server_data[WICED_BT_HFP_HF_MAX_CONN];
...
extern uint16_t wiced_bt_hfp_hf_get_uuid_by_handle(uint16_t handle);
extern uint16_t wiced_bt_hfp_hf_get_uuid_by_scn(uint8_t handle);
...
/* SLC related for HSP */
extern void wiced_bt_hsp_at_slc( wiced_bt_hfp_hf_scb_t *p_scb );
