...
uint8_t                     ag_num_scb;
hfp_ag_hci_send_ag_event_cback_t hfp_ag_hci_send_ag_event = NULL;
 ...
 */
void hfp_ag_startup( hfp_ag_session_cb_t *p_scb, uint8_t num_scb, uint32_t features, hfp_ag_hci_send_ag_event_cback_t p_app_cback )
{
    uint8_t i;
 ...
 */
void hfp_ag_connect( wiced_bt_device_address_t bd_addr )
{
    hfp_ag_session_cb_t *p_scb = ag_p_scb;
    uint8_t i;
...
void hfp_ag_audio_open( uint16_t handle )
{
    ...
    if ( p_scb->b_sco_opened )
    {
        ...
        if(hfp_ag_hci_send_ag_event)
            hfp_ag_hci_send_ag_event( HCI_CONTROL_AG_EVENT_AUDIO_OPEN, handle, (hfp_ag_event_t*)&ap_event );
        return;
    }
