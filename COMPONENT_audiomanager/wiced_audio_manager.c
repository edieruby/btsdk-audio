...
wiced_result_t wiced_am_stream_set_param(int32_t stream_id, uint32_t param_type, void *param_config)
{
    ...
    if ((0 <= stream_id) && (stream_id < MAX_NO_OF_STREAMS))
    {
        ...
        case AM_SAMPLE_RATE:
            ...
            break;
        case AM_NREC_CONFIG:
            /*Only HFP stream can use NREC*/
            if (wiced_am_info.stream[stream_id].stream_type == HFP)
            {
                /*if no explicit param is set we follow the basic stream param */
                if(param_config != NULL)
                    effect_conf.nrec_conf.sample_rate = ((nrec_config_t*)param_config)->sample_rate;
                else
                    effect_conf.nrec_conf.sample_rate = wiced_am_info.stream[stream_id].audio_config.sr;

                if (WICED_SUCCESS != audio_effect_init(PLATFORM_AUD_EFFECT_NREC,&effect_conf,&wiced_am_info.nrec_effect_id))
                {
                    WICED_BT_TRACE("platform_audio_effect_configure failed\n");
                    return WICED_ERROR;
                }
            }
            break;
        case AM_NREC_ENABLE:
            if (wiced_am_info.stream[stream_id].stream_type == HFP)
            {
                if (WICED_SUCCESS != audio_effect_ioctl(wiced_am_info.nrec_effect_id,PLATFORM_IOCTL_EFFECT_ENABLE,NULL))
                {
                    WICED_BT_TRACE(" NREC audio_effect_enable failed\n");
                    return WICED_ERROR;
                }
            }
            break;
        case AM_NREC_DISABLE:
            if (wiced_am_info.stream[stream_id].stream_type == HFP)
            {
                if (WICED_SUCCESS != audio_effect_ioctl(wiced_am_info.nrec_effect_id,PLATFORM_IOCTL_EFFECT_DISABLE,NULL))
                {
                    WICED_BT_TRACE(" NREC audio_effect_enable failed\n");
                    return WICED_ERROR;
                }

                if( WICED_SUCCESS != audio_effect_deinit(wiced_am_info.nrec_effect_id))
                {
                    WICED_BT_TRACE("platform_audio_effect_deint failed\n");
                    return WICED_ERROR;
                }
            }
            break;
